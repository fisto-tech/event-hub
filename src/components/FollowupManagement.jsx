    ...
    <div className="space-y-6 pb-10">
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col md:flex-row gap-4 items-end">
        <div className="flex-1">
          <label className="block text-sm font-bold text-crm-primary mb-1">Filter Follow-ups</label>
          <div className="flex bg-gray-100 rounded-lg p-1 w-full md:w-max border border-gray-200 overflow-x-auto scrollbar-none whitespace-nowrap">
            {['all', 'today', 'upcoming', 'missed'].map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 md:px-6 py-2 rounded-md font-black text-sm capitalize transition-all whitespace-nowrap ${
                  filter === f 
        ...
        <div className="w-full md:w-64">
          <label className="block text-sm font-bold text-crm-primary mb-1">Date Wise Analysis</label>
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap text-crm-textDark">
          <thead>
            <tr className="bg-gray-100 border-b border-gray-200">
              <th className="px-4 py-3 text-crm-primary font-black">Follow-up Date</th>
              <th className="px-4 py-3 text-crm-primary font-black">Customer Name</th>
              <th className="px-4 py-3 text-crm-primary font-black">Company</th>
              <th className="px-4 py-3 text-crm-primary font-black">Phone</th>
              <th className="px-4 py-3 text-crm-primary font-black">Status</th>
              <th className="px-4 py-3 text-crm-primary font-black">Notes</th>
          ...
          <tbody>
            {followups.map(f => (
              <tr key={f.id} className="border-b border-gray-100 hover:bg-crm-primaryLighter transition-colors">
                <td className="px-4 py-3 font-bold text-crm-primary">{f.follow_up_date}</td>
                <td className="px-4 py-3 font-black">{f.customer_name}</td>
