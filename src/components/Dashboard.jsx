        ...
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-shadow flex items-center justify-between">
            <div>
              ...
              <h3 className="text-4xl font-black text-crm-primary">{stat.value}</h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col h-96">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 className="font-black text-crm-primary text-lg">Recent Registrations</h3>
            <button className="text-sm font-bold text-crm-primary hover:text-crm-primaryDark transition-colors">View All</button>
          </div>
          {data.recentActivity && data.recentActivity.length > 0 ? (
              ...
              <ul className="divide-y divide-gray-100">
                {data.recentActivity.map((activity, idx) => (
                  <li key={idx} className="p-4 hover:bg-crm-primaryLighter transition-colors">
                    <p className="text-crm-textDark font-bold text-sm">{activity.company_name}</p>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              ...
              <p className="font-bold text-gray-600">No recent registrations</p>
              <p className="text-sm mt-1 text-gray-400 font-medium">New customers will appear here</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col h-96">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
            <h3 className="font-black text-crm-primary text-lg">Upcoming Follow-up List</h3>
          </div>
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
            <i className="ph ph-calendar-blank text-5xl mb-3 text-gray-400"></i>
            <p className="font-bold text-gray-600">You are all caught up!</p>
            <p className="text-sm mt-1 text-gray-400 font-medium">No pending follow-ups for today.</p>
          </div>
        </div>
      </div>
    </div>
